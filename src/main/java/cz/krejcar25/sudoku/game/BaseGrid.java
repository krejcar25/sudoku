...
public class BaseGrid extends Drawable {
	...
	private final int neighbourFill;
	private final int thisFill;
	private final int buttonFill;
	private final int buttonStroke;
	private final int flashFillBad;
	private final int flashFillGood;
	private final int darkBgFore;
	private final int lightBgFore;
	...
	private final Point orderTogglePos;
	private final Point deletePos;
	private final Point smallNumPos;
	private final Point settingsPos;
	private final Point exitPos;
	...
	private void setClicks() {
		...
		helpClick = () ->
		{
			...
			if (count < 1) flashSquare(helpPos.x, helpPos.y, 40, BAD);
			else if (count > 1) flashSquare(helpPos.x, helpPos.y, 40, WARN);
			else flashSquare(helpPos.x, helpPos.y, 40, GOOD);
		};
		orderToggleClick = () ->
		{
			numFirst = !numFirst;
	...
	void flashSquare(int x, int y, int flashTime, @MagicConstant(intValues = {GOOD, BAD, WARN}) int mood) {
		switch (mood) {
			case GOOD:
				flashSquares.newNow(x, y, flashTime, flashFillGood);
				break;
			case WARN:
				flashSquares.newNow(x, y, flashTime, neighbourFill);
				break;
			case BAD:
				flashSquares.newNow(x, y, flashTime, flashFillBad);
				break;
		}
	}
