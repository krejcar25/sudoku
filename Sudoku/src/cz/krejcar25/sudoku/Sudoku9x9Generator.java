...
public class Sudoku9x9Generator extends BaseGenerator {
    ...
    public Grid9x9 generate() {
        ...
        while (true) {
            if (available(gx, gy)) {
                int n = PApplet.floor(Main.pa.random(numbers.get(gx).get(gy).size()));
        ...
        Main.pa.println("removal target: " + (80 - targetCount) + ", removed: " + removed);
        game.lockAsBase(true, true);
        timer.stop();
        Main.pa.println("Generation finished in " + timer.getElapsedTimeSecs() + " seconds (" + timer.getElapsedTime() + " milliseconds, to be precise)");
        game.timer.start();
        return game;
    }
